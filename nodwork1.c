...
/* [XVT -> Qt PORT addition] Guard for updateBlockDiagram/updateSection/
 * updateLineMap/calculateAnomalies below: a Noddy model's history must
 * always start with a Stratigraphy event (same invariant already checked
 * by updateFloatingMenu's firstObject->shape == STRATIGRAPHY in
 * builder.c) -- generating a diagram or geophysics calculation before one
 * exists either crashes or produces a meaningless empty result. */
static BOOLEAN
requireStratigraphyDefined (void)
{
   WINDOW_INFO *wip = get_win_info (getEventDrawingWindow ());
   if (!wip || !wip->head || wip->head->shape != STRATIGRAPHY)
   {
      xvt_dm_post_error ("Please define a Stratigraphy event in the History window first.");
      return FALSE;
   }
   return TRUE;
}
...
#if (XVTWS == MACWS)
   if (!xvt_image_write_macpict_to_iostr (image, stream))
#else                                               
   ...
   { 
      if (!(lineMapData = (SECTION_DATA *) xvt_mem_zalloc (sizeof(SECTION_DATA))))
      {
         ...
         return;
                            ...
                            /* create a window if we need it */
                  /* [Qt port fix] was MENU_BAR_2 -- see updateSection's
                  ** matching comment; skip the per-window menu bar
                  ** entirely (same treatment as Block Diagram windows). */
      if (!(lineMapWindow = xvt_win_create (W_DOC, &position,
                   LINEMAP_TITLE, (int) NULL, TASK_WIN,
                   WSF_NO_MENUBAR | WSF_CLOSE | WSF_SIZE | WSF_INVISIBLE |
...
#endif
{
   ...
   xvt_vobj_get_client_rect (win, &position);
                  /* [Qt port fix] this window has its own menu bar
                  ** (MENU_BAR_2), so xvt_vobj_get_client_rect's rect has
                  ** .top offset down by the bar's height (needed so E_UPDATE
                  ** blits land below the bar, not hidden under it). Passing
                  ** that rect straight to xvt_vobj_move -- which sets the
                  ** window's actual outer position/size -- shifted this
                  ** window down by the bar height and undersized it by the
                  ** same amount every time a section/map gets (re)drawn,
                  ** i.e. on every update -- matches a user report of
                  ** Section/Map windows going blank. Content widgets nested
                  ** in this port's MDI model always want to sit at (0,0)
                  ** within their own subwindow frame; only the SIZE from
                  ** the client rect is meaningful here. */
...
#else
                     /* [User-requested format change] savePixmap's writer
                     ** now emits PNG, not BMP -- was reported producing
                     ** corrupted/unreadable frames; named the per-frame
                     ** files to match the real content. */
   addFileExtention (currentFile.name, ".png");
   strcpy (currentFile.type, "png");
#endif
...
#if (XVTWS == MACWS) || (XVTWS == WINWS) || (XVTWS == WIN16WS) || (XVTWS == WIN32WS)
                   TASK_MENUBAR, TASK_WIN, WSF_CLOSE | WSF_SIZE |
                   WSF_INVISIBLE | WSF_ICONIZABLE,
#else
                   (int) NULL, TASK_WIN,
                   WSF_NO_MENUBAR | WSF_CLOSE | WSF_SIZE |
