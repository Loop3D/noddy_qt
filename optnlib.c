 ...
 *
 * ONE deliberate exception, flagged here, same as eventlib.c's:
 * createOptions()'s groupXPos local was previously left GENUINELY
 * UNINITIALIZED under this port -- only assigned inside `#if XVTWS ==
 * WINWS`/etc blocks, which this port's XVTWS (deliberately 0, see
 * qt_compat/xvt_env.h) never matches. groupYPos gets an unconditional
 * `= 0` fallback a few lines down in the original code, but groupXPos has
 * no such fallback, so every OPTIONS_WINDOW content sub-panel (Project,
 * Block, Geology Display, etc) was positioned starting at an uninitialized
 * x-coordinate. Added a Qt-port fallback branch below using the same pixel
 * value as the WINWS case. Not a calculation change.
 * ============================================================================
 */
