...
/* Casts a pointer to the `long` xvt_vobj_set_data expects, round-trippable
 * back to a pointer with the reverse cast at the read site. */
#define PTR_LONG(p) ((long)(intptr_t)(p))
...
typedef long PICTURE;         /* encapsulated picture/clipboard descriptor */
