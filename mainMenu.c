...
#endif
{
	...
	** generates a new random one via the ported RandomNoddy.c. */
...
#else
		                     /* [User-requested format change] savePixmap's
		                     ** writer now emits PNG, not BMP -- named the
		                     ** file to match (was ".bmp", which produced a
		                     ** file with the wrong extension for its real
		                     ** content). */
		            addFileExtention (fs.name, ".png");
#endif
		            incrementFilename(&fs);
		         }
		         else
	 ...
	 * removed; TASK_MENUBAR_5_120_121_123/124/122_125/126 handlers
	 * (First/Second Gravity/Magnetics, calcVertDerivImage) removed with
	 * it since their menu items no longer exist. */
