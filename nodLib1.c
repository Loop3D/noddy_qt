...
#define PASTE_OFFSET 64 /* [Qt port] icon-spacing-ish offset for Paste/Duplicate placement, see pasteObjects() -- was 40, still overlapped the icon+title label of the original, per user screenshot */
#define NUM_MENUS 6
...
#endif
{
   ...
   xvt_vobj_get_client_rect (pixmap, &position);
                  ...
                  /* [Qt port fix] position is the CONTENT pixmap's size
                  ** (no allowance for a per-window menu bar). Windows that
                  ** carry one (Section/Map, via createLineMapMenubar) need
                  ** extra height reserved for it, or Default Size squeezes
                  ** their content under/behind the bar -- user-reported as
                  ** "Default Size moves image above its menu bar ... for
                  ** most windows except Block Diagram" (which has no menu
                  ** bar, hence unaffected). xvt_vobj_get_client_rect(win,..)
                  ** reports its client area starting at .top == the menu
                  ** bar's height (0 if there isn't one), so reuse that. */
   {
      RCT winClientRect;
      xvt_vobj_get_client_rect (win, &winClientRect);
      position.bottom += winClientRect.top;
   }
...
RETURNED
====================================================================== */
/* [Qt port] set by duplicateSelectedObjects() immediately before calling
 * pasteObjects(), so the one shared function can place its result
 * differently for the two commands -- see pasteObjects' comment below. */
static BOOLEAN pasteAsDuplicate = FALSE;
...
#endif
{
   ...
   OBJECT *p, *lastExisting = NULL;
   int dh, dv;
   BOOLEAN inPastedGroup;
...
DEBUG(printf("PASTING OBJECTS - GOT THEM");)
   ...
   for (p = objectList; p != NULL; p = p->next)
      copyUndoAdditions (p, FALSE);
          ...
          /* [Qt port fix] real XVT synthesized an E_MOUSE_DOWN at the
          ** pasted objects' own (copied-from-source) position and relied
          ** on that kicking off the same interactive "drag to place" flow
          ** as a manual move -- doMouse's DR_MOVE handling never actually
          ** got a corresponding real mouse-up in this port, so the pasted
          ** copies just sat exactly on top of what they were copied from.
          ** User-requested explicit behaviour instead: Paste lands to the
          ** right of the current end of the chain; Duplicate lands
          ** directly below the object(s) it duplicated. Shift the whole
          ** pasted group by the same delta so multi-object copies keep
          ** their relative layout. */
   if (pasteAsDuplicate || !lastExisting)
   {
      dh = 0;
      dv = PASTE_OFFSET;
   }
   ...
   {
      dh = (lastExisting->bound.right - objectList->bound.left) + PASTE_OFFSET;
      dv = 0;
   }
   if (dh || dv)
   {
      for (p = objectList; p != NULL; p = p->next)
      {
         p->bound.left   += dh; p->bound.right  += dh;
         p->bound.top    += dv; p->bound.bottom += dv;
      }
   }

   /* [Qt port] Duplicate: the new copy always lands directly below the
   ** original at a fixed offset, so duplicating the same event twice (or
   ** duplicating one that already has something sitting just below it)
   ...
   ** knock-on overlap with the next one below it. */
   if (pasteAsDuplicate)
   {
      ...
      dupBound = objectList->bound;
      for (p = objectList->next; p != NULL; p = p->next)
      {
         if (p->bound.left   < dupBound.left)   dupBound.left   = p->bound.left;
         if (p->bound.top    < dupBound.top)    dupBound.top    = p->bound.top;
         if (p->bound.right  > dupBound.right)  dupBound.right  = p->bound.right;
         if (p->bound.bottom > dupBound.bottom) dupBound.bottom = p->bound.bottom;
      }
      dupHeight = dupBound.bottom - dupBound.top;

      do
      {
         RCT overlap;
         pushed = FALSE;
         for (other = wip->head; other != NULL; other = other->next)
         {
            if (other->shape == STOP)
               continue;
            inPastedGroup = FALSE;
            for (p = objectList; p != NULL; p = p->next)
               if (p == other) { inPastedGroup = TRUE; break; }
            if (inPastedGroup)
               continue;

            if (xvt_rect_intersect (&overlap, &dupBound, &(other->bound)))
            {
               other->bound.top    += dupHeight + PASTE_OFFSET;
               other->bound.bottom += dupHeight + PASTE_OFFSET;
               pushed = TRUE;
            }
         }
      } while (pushed);
...
#endif
{
   if (copySelectedObjects (win))
   {
      pasteAsDuplicate = TRUE;
      pasteObjects (win);
      pasteAsDuplicate = FALSE;
   }
}
