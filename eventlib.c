 ...
 *
 * ONE deliberate exception, flagged here: createEventOptions()'s groupXPos/
 * groupYPos locals were previously left GENUINELY UNINITIALIZED under this
 * port. The original code only assigns them inside `#if XVTWS == WINWS` /
 * `MACWS` / `MTFWS` / `XOLWS` blocks, assuming one always matches; qt_compat/
 * xvt_env.h deliberately defines XVTWS to match NONE of those (to correctly
 * skip the many genuinely non-portable Win32/Mac/Motif escape hatches
 * elsewhere in this codebase), which incidentally also skipped this block --
 * but this specific block isn't an escape hatch, it's the ONLY place these
 * layout constants are set, with no portable fallback. Left as-is, every
 * event dialog's content sub-panel (Fold's Form/Scale, Stratigraphy's layer
 * list, etc) was positioned using uninitialized stack garbage, causing
 * `short` overflow in the resulting window rects and effectively-invisible
 * (zero-size or wildly mispositioned) content -- confirmed via gdb tracing
 * (groupYPos read back as 32767, i.e. SHRT_MAX, a classic uninitialized-
 * variable signature). Added a Qt-port fallback branch below using the same
 * pixel values as the WINWS case, since this port's UI targets a Windows-
 * style MDI layout (see qt_compat/xvt_compat.cpp's QMdiArea-based window
 * nesting). Not a calculation change.
 * ============================================================================
 */
