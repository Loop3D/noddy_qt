   ...
   switch (ep->type)
   {
      ...
      case E_SIZE:
         if (!xvt_vobj_get_data (win))
           break;
         xvt_vobj_get_client_rect (win, &position);
                     /* [Qt port fix] this window carries its own menu bar
                     ** (MENU_BAR_2), so the client rect's .top is offset
                     ** down by the bar's height (for correct E_UPDATE blit
                     ** alignment below the bar). position.left/top get
                     ** reused below as the anchor for the resized rect
                     ** passed straight to xvt_vobj_move -- left un-zeroed,
                     ** that shifted this window down by the bar height and
                     ** undersized it by the same amount on every resize,
                     ** matching a user report of Section/Map windows going
                     ** blank. Normalize to zero-origin, preserving size. */
...
#define MIN_SPIN_MOVEMENT  3.0   /* Number of pixels to move in a direction to spin block */
   ...
   switch (ep->type)
   {
      ...
      case E_SIZE:
           ...
           xvt_dwin_invalidate_rect (win, NULL);
                        /* [Reverted -- user-requested "force refresh on
                        ** resize" feature caused an infinite E_SIZE loop:
                        ** updateBlockDiagram() ends up re-triggering a
                        ** resize on this same window, which fires E_SIZE
                        ** again, which calls updateBlockDiagram() again,
                        ** forever. Back to real XVT's original behaviour
                        ** (cheap stretch-blit only, no recompute here). */
           break;
      case E_UPDATE:
           if (!xvt_vobj_get_data (win))
              break;
